...
fn App() -> Element {
            ...
            if let Some(path) = rfd::FileDialog::new().add_filter("BLF Files", &["blf", "bin"]).pick_file() {
                ...
                match read_blf_from_file(&path) {
                    Ok(result) => {
                        ...
                        let time_opt = chrono::NaiveTime::from_hms_milli_opt(st.hour as u32, st.minute as u32, st.second as u32, st.milliseconds as u32);
    ...
    let handle_load_config_click = move |_| {
        spawn(async move {
            if let Some(path) = rfd::FileDialog::new().add_filter("Config Files", &["json"]).pick_file() {
                status_msg.set(format!("Loading config..."));
                if let Ok(content) = std::fs::read_to_string(&path) {
                    match serde_json::from_str::<AppConfig>(&content) {
                        Ok(config) => {
                            app_config.set(config);
                            config_dir.set(Some(path.parent().unwrap_or(std::path::Path::new(".")).to_path_buf()));
...
#[component]
fn MessageRow(msg: LogObject, dbc_channels: Signal<HashMap<u16, DbcDatabase>>, ldf_channels: Signal<HashMap<u16, LdfDatabase>>) -> Element {
            ...
            m.data.iter().take(m.dlc as usize).map(|b| format!("{:02X}", b)).collect::<Vec<_>>().join(" "),
            {
                if let Some(db) = dbc_channels.read().get(&m.channel) {
                    if let Some(dbc_msg) = db.messages.get(&m.id) {
                        let signals: Vec<String> = dbc_msg.signals.values().map(|s| {
                            format!("{}: {:.2}{}", s.name, s.decode(&m.data), if s.unit.is_empty() { "".to_string() } else { format!(" {}", s.unit) })
                        }).collect();
            ...
            m.data.iter().take(m.dlc as usize).map(|b| format!("{:02X}", b)).collect::<Vec<_>>().join(" "),
            {
                 if let Some(db) = ldf_channels.read().get(&m.channel) {
                     if let Some(frame) = db.frames.values().find(|f| f.id == m.id as u32) {
                         let signals: Vec<String> = frame.signals.iter().map(|mapping| {
                ...
                td {
                    ...
                    span {
                        style: format!("padding: 2px 6px; border-radius: 4px; font-size: 11px; font-weight: bold; background: {}; color: white;",
                            if type_name == "CAN" { "#0078d4" } else { "#6f42c1" }
                        ),
                        "{type_name}"
                    }
                }
...
fn ChartView(
       ...
       if let Some(t0) = start_time_val {
           ...
           for msg in msgs.iter() {
               let (channel, id, data, timestamp_ns) = match msg {
                   LogObject::CanMessage(m) => (m.channel, m.id, &m.data[..], m.header.object_time_stamp),
                   LogObject::CanMessage2(m) => (m.channel, m.id, &m.data[..], m.header.object_time_stamp),
                   LogObject::CanFdMessage(m) => (m.channel, m.id, &m.data[..], m.header.object_time_stamp),
                   _ => continue,
               };
           ...
           if has_data {
                ...
                if y_padding == 0.0 { min_y -= 1.0; max_y += 1.0; } else { min_y -= y_padding; max_y += y_padding; }
                ...
                let min_t_ts = min_t.and_utc().timestamp() as f64 + min_t.and_utc().timestamp_subsec_nanos() as f64 / 1_000_000_000.0;
                let max_t_ts = max_t.and_utc().timestamp() as f64 + max_t.and_utc().timestamp_subsec_nanos() as f64 / 1_000_000_000.0;
                ...
                for (key, points) in plot_data.iter() {
                    let converted_points: Vec<(f64, f64)> = points.iter()
                        .map(|(dt, y)| (dt.and_utc().timestamp() as f64 + dt.and_utc().timestamp_subsec_nanos() as f64 / 1_000_000_000.0, *y))
                        .collect();
               ...
               let mut chart = ChartBuilder::on(&root)
                   .caption(format!("Signal Plot (Start: {})", t0.format("%Y-%m-%d %H:%M:%S")), ("sans-serif", 20).into_font())
                   .margin(10)
                   .x_label_area_size(40)
                   .y_label_area_size(50)
                   .build_cartesian_2d(min_t_ts..max_t_ts, min_y..max_y).unwrap();
           ...
           for msg in msgs.iter() {
                   ...
                   if let Some(dbc_msg) = db.messages.get(&id) {
                       for sig in dbc_msg.signals.values() {
                           ...
                           if let Some(data_vec) = plot_data.get_mut(&key) {
                               ...
                               has_data = true;
                               if time < min_x { min_x = time; }
                               if time > max_x { max_x = time; }
                               if value < min_y { min_y = value; }
                               if value > max_y { max_y = value; }
                           }
                       }
            ...
            if has_data {
               ...
               let mut chart = ChartBuilder::on(&root)
                   .caption("Signal Plot (Relative Time)", ("sans-serif", 30).into_font())
                   .margin(10)
                   .x_label_area_size(40)
                   .y_label_area_size(50)
    ...
    let mut ui_channels = channels_data.into_iter().map(|(id, db)| {
        let mut msgs: Vec<_> = db.messages.values().map(|m| {
            let mut sigs: Vec<_> = m.signals.values().map(|s| {
                (s.name.clone(), format!("{}:{}:{}", id, m.id, s.name))
            }).collect();
