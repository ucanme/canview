...
pub fn serialize_file_statistics(stats: &FileStatistics) -> Vec<u8> {
    ...
    writer.write_u8(0).unwrap(); // compressionLevel (0 for uncompressed)
...
fn main() {
    let start_time = SystemTime {
        year: 2025, month: 12, day_of_week: 6, day: 27,
        hour: 22, minute: 0, second: 0, milliseconds: 0
    };
    ...
    for i in 0..5 {
        ...
        let msg = CanMessage {
            ...
            channel: 1, flags: 0, dlc: 8, id: 0x101, data,
        };
    ...
    for i in 0..5 {
        ...
        let msg = CanMessage {
            ...
            channel: 2, flags: 0, dlc: 4, id: 0x201, data,
        };
        let mut m_bytes = serialize_can_message(&msg);
        add_padding(&mut m_bytes);
        inner_data.extend(m_bytes);
    ...
    for i in 0..10 {
        ...
        let msg = CanMessage {
            ...
            channel: 1, flags: 0, dlc: 8, id: 0x100 + i as u32, data,
        };
        let mut m_bytes = serialize_can_message(&msg);
        add_padding(&mut m_bytes);
        inner_data.extend(m_bytes);
    ...
    let container_header = ObjectHeader {
        ...
        header_size: 32,
        header_version: 1,
        object_size: (32 + 16 + inner_data.len()) as u32,
    ...
    container_writer.write_u32::<LittleEndian>(inner_data.len() as u32).unwrap();
